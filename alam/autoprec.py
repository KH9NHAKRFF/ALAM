...
import numpy as np
from alam.utils import exp_recorder
import alam.cpp_extension.calc_precision as ext_calc_precision
...
class AutoPrecision:
    ...
    def iterate(self, backprop):
        ...
        if self.iter == 0 or self.iter == self.adapt_step :
            # Do full adaptation
            print('ALAM: Initializing AutoPrec..., run extra %d iters' %
    ...
    def refresh_bits(self):
        ...
        if self.log_iter > 0 and self.iter > self.warmpup_iter:
            ...
            if quantization_var > overall_var * 0.1:
                print("========================================")
                print('ALAM Warning: Quantization variance is too large. Consider increasing number of bits.',
